...
let errorTypeChart = null; // 错误类型分布图实例
 ...
 */
function getErrorTypeNodeFilter() {
    return document.getElementById("error-type-node-filter");
}
/**
 * 将错误类型名称归一为可展示文本。
 ...
 */
function getErrorTypeColor(index) {
 ...
 */
function getEmptyErrorTypeColor() {
 ...
 * @returns {void}
 */
export function initErrorTypeChart() {
 ...
 * @returns {void}
 */
function renderErrorTypeLegend() {
 ...
 * @returns {void}
 */
export function renderErrorTypeChart() {
    if (!errorTypeChart) {
        initErrorTypeChart();
    }
    if (!errorTypeChart)
 ...
 * @returns {void}
 */
export function populateErrorTypeNodeFilter(statuses) {
    const filterEl = getErrorTypeNodeFilter();
    if (!filterEl)
        return;
