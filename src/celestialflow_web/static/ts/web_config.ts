 ...
 */
export function normalizeErrorColumns(
 ...
 */
function normalizeStructureEdgeLabel(value: unknown): StructureEdgeLabel {
  ...
  if (isGroupedWebConfig(rawConfig)) {
    return {
      ...
      dashboard: {
        ...DEFAULT_WEB_CONFIG.dashboard,
        ...(rawConfig.dashboard ?? {}),
        structureEdgeLabel: normalizeStructureEdgeLabel(
          rawConfig.dashboard?.structureEdgeLabel,
        ),
...
// 全局状态
export let webConfig: WebConfig = normalizeWebConfig(); // 当前加载的 Web 配置
export let refreshRate = 5000; // 轮询刷新间隔（毫秒），为 webConfig.global.refreshInterval 的归一化值

/** 设置轮询刷新间隔（供主入口的刷新间隔下拉框跨模块写入） */
export function setRefreshRate(value: number): void {
  refreshRate = value;
}
...
export const CARD_META: Record<string, string> = {
 ...
 */
export async function saveWebConfig(): Promise<boolean> {
...
export function applyConfig(): void {
  ...
  // 应用结构图边标签显示模式
  webConfig.dashboard.structureEdgeLabel = normalizeStructureEdgeLabel(
    webConfig.dashboard.structureEdgeLabel,
  );
